...
### 未登録の作品を、その場で追加してそのまま記録する（2026-08-21 実装）

- **背景**：検索して見つからない絵本・紙芝居は記録できず、そこで行き止まりだった。
  009・010（適用済み）で「仮登録するRPC」までは用意できていたが、
  画面からの入口が無く、利用者は一度も使えなかった。
- **実装**：ホームの「絵本・紙芝居を探す」の検索結果の下に「この作品を追加する」を追加
  （`src/components/AddWorkPanel.jsx`）。追加が終わると検索し直させず、
  `book_id` を持った状態で記録画面へ進む。
- **books へは直接書かない**。追加は `create_provisional_book` RPC だけを通す
  （`dataAdapter.createProvisionalBook`）。RPCが返す3つの状態を画面で扱う：
  `existing`＝既存のIDで記録へ／`created`＝新しいIDで記録へ／
  `candidates`＝似た作品を出して人に選んでもらう（「どれでもない」のときだけ新規作成）。
- **仮登録した作品はすぐ記録できる**。`practice_logs` のRLSは `user_id = auth.uid()` だけを見ており、
  `book_id` の外部キーに `is_active` の条件は無い（管理者の公開を待たせない）。
- **絵本／紙芝居**：書名・シリーズ名から紙芝居と分かるときだけ自動で決め、
  分からないときだけ選んでもらう（`normalize.detectMaterialType`）。

### 自分が追加した作品を、翌日も通常検索から選べるようにした（2026-08-21）

- **問題**：仮登録した作品は `is_active=false` のため通常検索に出ず、翌日また
  「まだ登録されていません → この作品を追加する」からやり直す必要があった。
  重複登録はRPC側が防ぐのでデータは正しいが、毎回遠回りになっていた。
- **考え方**：`is_active` は「みんなに公開してよいか」の旗であって、
  「本人が使ってよいか」ではない。公開作品の検索条件は変えず、
  **自分が追加した分だけを足して見せる**。
- **他人の追加分が混ざらない理由**：`books` を直接見に行かず、
  `book_contributions` から作品を辿る。この表のRLS `contrib_select_own` は
  `created_by = auth.uid()` の行しか読ませないため、構造上取得できない。
- **匿名ユーザーを増やさない**：検索では `ensureAnonymousSession` を呼ばない。
  既にセッションがあるときだけ自分の追加分を確認する。
- **画面**：「登録されている絵本」と「自分が追加した作品」を別の見出しで出す。
  「未公開」「確認待ち」等の管理側の言葉は出さない。選んだあとは既存どおり
  詳細 →「この絵本を記録する」→ 記録画面 → 保存。
- **DB変更なし**（RLS・RPC・migrationはいずれも変更していない）。

---

### 実Supabaseでの通し確認（2026-08-21 実施・Master承認のうえ実DBへ書き込み）

画面を実際に操作し、アプリがSupabaseへ送受信した内容を確認した結果。以下はすべて実測。

**① 既存作品の記録**
- 実DBの「ぐりとぐら」を検索 → 詳細 →「この絵本を記録する」→ 記録画面 → 保存に成功
- `practice_logs.book_id` は既存作品のID `22222222-0000-0000-0000-000000000002` と一致
- 保存後、記録一覧に正しく表示された

**② 未登録の実在作品の追加**
- 実DBに無い実在作品「きんぎょがにげた」（五味太郎）を追加
- `POST /rest/v1/rpc/create_provisional_book` → `200`、`status: "created"`、
  `book_id: 43c23d14-854c-4881-b367-96e882147c40`
- **`books` への直接INSERTは行われていない**（`POST /rest/v1/books` は0回）
- `book_contributions` が1件作成された（id `b3ebb202-7e99-4fb2-a73c-3c1103671a94`）
- 仮登録の直後に、同じ book_id で `practice_logs` を保存できた（`201`）

**③ 再検索**
- ホームへ戻って同じ作品名で検索すると「自分が追加した作品」として表示された
- 公開作品の検索（`books?is_active=eq.true`）の結果は `[]` のまま
- そこから詳細 →「この絵本を記録する」→ 記録画面へ進め、同じ book_id が渡った

**④ 重複防止**
- 同じ作品をもう一度追加しようとすると `status: "candidates"` が返り、`book_id` は `null`
- 重複した `books` の行は作成されなかった

**⑤ エラー**
- RLSエラー・RPCエラー・コンソールエラーはいずれも発生しなかった

**⑥ 検証データの扱い（2026-08-25 実施）**
- 検証用に作成した `practice_logs` 2件
  （`97255249-84ca-4d60-aa7e-9349ff73e355` / `76f87a51-bbfa-4dc2-b4d4-a3d4fc1c5bc2`）は
  匿名ユーザーのセッションから削除済み（RLSが効いた通常の経路。SQL Editor・service_roleは未使用）
- 削除後、この利用者の `practice_logs` は0件
- 「きんぎょがにげた」の `books` 行と `book_contributions` の行は**残している**
- 「きんぎょがにげた」は現在 `is_active = false`（公開の判断は管理者が行う）

---

### 🔴 外部書誌の取得元について分かったこと（2026-08-21 実測）

| 取得元 | ブラウザから直接呼べるか | 実測した事実 |
|---|---|---|
| openBD | **呼べる** | `access-control-allow-origin: *` を確認。ただし**ISBNでしか引けない**（書名検索のAPIが無い） |
| 国立国会図書館サーチ | **呼べない** | `/api/opensearch` の応答にCORSヘッダが無い。ブラウザからは遮断される |

つまり **書名での外部検索は、ブラウザだけでは実現できない**。
現在は「ISBN（本の裏のバーコードの数字）を入れたときは openBD から書誌を取得し、
それ以外は手入力で追加する」形にしてある。
書名で外部検索できるようにするには、Supabase Edge Function などの
**サーバ側の中継が必要**（Master判断待ち）。
